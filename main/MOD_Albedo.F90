...
!-----------------------------------------------------------------------
  ...
  SUBROUTINE albland (ipatch, patchtype, deltim,&
                      ...
                      alb,ssun,ssha,ssno,thermk,extkb,extkd,albsp)
...
! ground cover index
 ...
 real(r8), intent(out) :: &
      ...
      albsp(190,2), &! reflectance spectrum
...
!-----------------------------------------------------------------------
!Calculate reflectance spectral parameter settings using broadband reflectance
! 0. Improvement content
!-----------------------------------------------------------------------
   CHARACTER(len=256) :: filename
   real(r8),dimension(:,:), allocatable :: Vlam(:,:)
   real(r8),dimension(:,:), allocatable :: Vlam_inv(:,:)
   integer :: m, n, stat
   n = 190
   m = 4
   allocate(Vlam(n,m), stat=stat)
   allocate(Vlam_inv(m,n), stat=stat)
   filename = '/stu01/sunwy22/fortran/albsim/Vlam.bin'
! ----------------------------------------------------------------------
! 1. Initial set
! ----------------------------------------------------------------------
...
! 2.3 albedo for inland water
      ELSE IF(patchtype >= 4) THEN
         ...
         ENDIF
      ENDIF
! 2.4 Calculating reflectance spectra using broadband reflectance
      call landalb(alb, filename, Vlam, Vlam_inv, albsp)
