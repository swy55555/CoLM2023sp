...
!End ozone stress variables
           ...
           allocate (albsp                   (190,2,numpatch)); albsp     (:,:,:) = spval
...
ENDIF
     ...
     CALL ncio_read_vector (file_restart, 'albsp   '   , 190, 2, landpatch, albsp  )
...
ENDIF
  ...
  SUBROUTINE check_TimeVariables ()
     ...
     CALL check_vector_data ('albsp       [-]    ', albsp        )
